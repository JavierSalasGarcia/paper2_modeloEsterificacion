...
El modelo simplificado representa la transesterificación como una reacción global reversible donde un mol de triglicérido reacciona con tres moles de metanol para producir tres moles de ésteres metílicos de ácidos grasos y un mol de glicerol:
...
El modelo completo descompone la transesterificación en tres reacciones consecutivas reversibles que capturan la formación de intermediarios diglicérido y monoglicérido:
...
Extensiones planificadas del sistema incluyen expansión de validación a catalizadores heterogéneos variados como MgO, SrO y BaO documentados en literatura, incorporación de catalizadores homogéneos NaOH y KOH con cinéticas de dos fases líquidas, desarrollo de interfaz gráfica web usando frameworks como Django o Flask para eliminar dependencia de línea de comandos, integración nativa con OpenFOAM para simulaciones CFD automatizadas del reactor escalado, incorporación de análisis económico calculando costos de capital y operación junto con tiempos de retorno de inversión, y creación de módulo de control de calidad evaluando cumplimiento de especificaciones de normas EN 14214 o ASTM D6751 para biodiésel basándose en predicciones de pureza de producto del modelo de 3-pasos.

Adicionalmente se considera extensión a variantes de proceso como transesterificación in situ donde extracción de aceite y transesterificación ocurren simultáneamente, transesterificación supercrítica sin catalizador a altas temperaturas y presiones, y transesterificación enzimática usando lipasas. Cada variante requeriría módulos cinéticos especializados pero podría reutilizar infraestructura existente de optimización, análisis de sensibilidad y escalado. La arquitectura modular fue diseñada anticipando estas extensiones mediante interfaces bien definidas entre componentes.

Colaboraciones con grupos experimentales permitirían validación extensa con datos independientes y refinamiento de parámetros cinéticos. El sistema podría servir como plataforma para repositorio comunitario de parámetros calibrados para diferentes combinaciones de aceite, alcohol y catalizador, facilitando meta-análisis y desarrollo de correlaciones predictivas basadas en propiedades moleculares. Esta visión de plataforma comunitaria requeriría desarrollo de infraestructura de base de datos y mecanismos de contribución que están fuera del alcance actual pero representan dirección estratégica a largo plazo.
...
El código fuente completo del sistema, casos de uso documentados, datos de entrada, scripts de ejecución y resultados están disponibles públicamente en:

\begin{center}
\texttt{https://github.com/JavierSalasGarcia/modelo\_esterificacion}
\end{center}

Los casos de uso se encuentran en el directorio \texttt{Casos/} con subdirectorios \texttt{caso1\_procesamiento\_gc/}, \texttt{caso2\_ajuste\_parametros/}, \texttt{caso3\_optimizacion/}, \texttt{caso4\_comparacion\_modelos/}, \texttt{caso5\_analisis\_sensibilidad/} y \texttt{caso6\_escalado\_reactores/}. Cada caso incluye archivos de configuración JSON, datos de entrada, scripts bash de ejecución y resultados completos incluyendo figuras.

El sistema está licenciado bajo MIT License permitiendo uso, modificación y distribución libre con atribución apropiada.
